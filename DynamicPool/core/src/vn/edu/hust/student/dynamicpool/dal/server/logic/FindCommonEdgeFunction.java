...
public class FindCommonEdgeFunction {
	...
	public static void findCommonEdge(List<PoolServer> pools) {
	...
	private static void initOrderOfPools(List<PoolServer> pools) {
		float lastX = 0;
		for (PoolServer poolServer : pools) {
			Rectangle poolPosition = poolServer.getCorrdiate();
			poolPosition.getLocation().setX(lastX);
			lastX += poolPosition.getWidth();
		}
	}
	
	private static void initPositionForPools(List<PoolServer> pools, int numberOfPools,
			float[][] xCoordinate, float[][] yCoordinate) {
		for (int i = 0; i<pools.size(); i++) {
			Rectangle corrdiate = pools.get(i).getCorrdiate();
	...
	private static void findSegments(List<PoolServer> pools, int numberOfPools,
			float[][] xCoordinate, float[][] yCoordinate) {
//		Line line = new Line();
